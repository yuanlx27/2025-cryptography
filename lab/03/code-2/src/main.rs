...
static TABLE: LazyLock<([u8; 256], [u8; 256])> = LazyLock::new(|| {
    ...
    table[0] = 1;
    for i in 1..256 {
        let mut temp = ((table[i - 1] as u16) << 1) ^ table[i - 1] as u16;
        if temp & 0x100 != 0 {
            temp ^= 0x11b;
        }
        table[i] = temp as u8;
...
fn main() {
    let mut reader: Box<dyn io::Read> = if cfg!(feature = "online_judge") {
        ...
        Box::new(fs::File::open("input.bin").unwrap())
    };
    let mut writer: Box<dyn io::Write> = if cfg!(feature = "online_judge") {
        Box::new(io::stdout())
    } else {
        Box::new(fs::File::create("output.bin").unwrap())
    };
    ...
    let result = match mode {
        0x01 => encrypt(&words, &iv, &text),
        0x81 => decrypt(&words, &iv, &text),
...
fn key_expansion(key: &[u8; 16]) -> [u32; 44] {
    ...
    for i in 4..44 {
        ...
        if i % 4 == 0 {
            temp = u32::from_be_bytes(sub_word(&rot_word(&temp.to_be_bytes()))) ^ ((RCON[i / 4] as u32) << 24);
        }
...
fn encrypt(words: &[[u8; 16]], iv: &[u8; 16], text: &[u8]) -> Vec<u8> {
    ...
    state.chunks(16).map(move |chunk| {
        ...
        for i in 0..4 {
            for j in 0..4 {
                state[i * 4 + j] = chunk[j * 4 + i] ^ last[i * 4 + j];
            }
        }
...
fn decrypt(words: &[[u8; 16]], iv: &[u8; 16], text: &[u8]) -> Vec<u8> {
    let mut last = *iv;
